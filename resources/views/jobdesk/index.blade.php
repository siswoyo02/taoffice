...
<div class="container-fluid">
    <div class="card card-outline card-primary">
        <div class="card-header">
            @can('admin')
            <center>
                <a href="{{ url('/jobdesk/create') }}" class="btn btn-primary"><i class="fa fa-plus"></i> Tambah Data jobdesk</a>
            </center>
            @endcan
        </div>
        <!-- /.card-header -->
        <div class="card-body">
            <table id="tableprint" class="table table-bordered table-striped">
                ...
                <tbody>
                    @foreach ($data_jobdesk as $jd)
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ $jd->nama_jobdesk }}</td>
                        <td>{{ $jd->alamat }}</td>
                        <td>{{ $jd->nama_project }}</td>
                        <td>
                            <a href="{{ url('/jobdesk/edit/'.$jd->id) }}" class="btn btn-sm btn-warning"><i class="fa fa-solid fa-edit"></i></a>
                            <form action="{{ url('/jobdesk/delete/'.$jd->id) }}" method="post" class="d-inline">
                                @method('delete')
                                @csrf
